...
const mockData = {
  ...
  auditLogs: [
    { id: 1, date: new Date().toISOString(), userName: 'Dr. Sarah Chen', action: 'CREATE_PRODUCT', details: 'Added Amoxicillin 500mg (SKU: AMX-500)' },
    { id: 2, date: new Date(Date.now() - 300000).toISOString(), userName: 'Marcus Vane', action: 'UPDATE_STOCK', details: 'Updated stock for Insulin Glargine: 45 \u2192 65' },
    { id: 3, date: new Date(Date.now() - 600000).toISOString(), userName: 'Dr. Sarah Chen', action: 'APPROVE_PO', details: 'Approved PO-2026-001 from Global Pharma Solutions' },
    { id: 4, date: new Date(Date.now() - 900000).toISOString(), userName: 'Elena Rodriguez', action: 'RECEIVE_SHIPMENT', details: 'Received shipment for PO-2026-002, 3 batches recorded' },
    { id: 5, date: new Date(Date.now() - 3600000).toISOString(), userName: 'Marcus Vane', action: 'UPDATE_PRODUCT', details: 'Changed low stock threshold for Paracetamol 500mg to 300' },
    { id: 6, date: new Date(Date.now() - 7200000).toISOString(), userName: 'Dr. Sarah Chen', action: 'CREATE_USER', details: 'Created user account for James Wilson (WAREHOUSE_STAFF)' },
    { id: 7, date: new Date(Date.now() - 86400000).toISOString(), userName: 'Marcus Vane', action: 'DELETE_BATCH', details: 'Expired batch LOT-E11 removed from Atorvastatin 20mg' },
  ]
};

// Helper for audit logs
const pushAuditLog = (action: string, userName: string, details: string) => {
  const currentLogsStr = localStorage.getItem('auditLogs');
  let logs = currentLogsStr ? JSON.parse(currentLogsStr) : [...mockData.auditLogs];
  logs.unshift({
    id: Date.now(),
...
const api = {
  auth: {
    login: async (email: any, password: any) => {
      try {
        ...
        if (email === 'admin@test.com' && password === 'password') {
          ...
          pushAuditLog('LOGIN', user.name, 'User authenticated successfully');
    ...
    register: async (name: any, email: any, password: any) => {
      try {
        await new Promise(resolve => setTimeout(resolve, 500));
        pushAuditLog('REGISTER', name, `New user registered with email: ${email}`);
        return { success: true };
      } catch (error) {
  ...
  audit: {
    ...
    getAll: async () => {
      try {
        ...
        const currentLogsStr = localStorage.getItem('auditLogs');
        return currentLogsStr ? JSON.parse(currentLogsStr) : mockData.auditLogs;
      } catch (error) {
        return handleApiError(error, 'audit.getAll');
      }
    }
  },
  users: {
    getAll: async () => {
      try {
        ...
        return handleApiError(error, 'users.getAll');
      }
    }
  },
  products: {
    getAll: async () => {
      try {
        ...
        return handleApiError(error, 'products.getAll');
      }
    },
    getById: async (id: any) => {
      try {
        ...
        return handleApiError(error, 'products.getById');
      }
    }
  },
  suppliers: {
    getAll: async () => {
      try {
        ...
        return handleApiError(error, 'suppliers.getAll');
      }
    }
  },
  purchaseOrders: {
    getAll: async () => {
      try {
        ...
        return handleApiError(error, 'purchaseOrders.getAll');
      }
    },
    getById: async (id: any) => {
      try {
        await new Promise(resolve => setTimeout(resolve, 300));
        const po = mockData.poDetails[id] || mockData.poDetails['PO-2026-001'];
        if (!po) throw new Error('Purchase Order not found');
        return po;
      } catch (error) {
        return handleApiError(error, 'purchaseOrders.getById');
      }
    }
  }
};
