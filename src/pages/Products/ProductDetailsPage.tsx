...
import { ArrowLeft, Package, Calendar, Tag, Info, Activity, ShieldCheck, Zap, ShieldAlert, Plus, Edit2, Save, History, BarChart3 } from 'lucide-react';
import api from '@/services/api';
import DataTable from '@/components/common/Tables/DataTable';
import EOQCard from '@/components/common/EOQCard';
...
const ProductDetailsPage = () => {
  ...
  return (
    <div className="fade-in">
      <div className="mb-4">
        <Link to="/dashboard/products" className="btn btn-link text-decoration-none d-flex align-items-center gap-2 p-0 text-info opacity-75 hover-opacity-100">
          <ArrowLeft size={18} />
      ...
      <div className="row g-4 mb-4">
        <div className="col-12">
          <EOQCard data={{ optimalOrderQty: 120, reorderPoint: 45, estimatedSavings: 1250 }} />
        </div>
      </div>
        ...
        <div className="col-lg-8">
          <div className="glass-card p-4 border-0 mb-4">
            ...
            <div className="list-group list-group-flush bg-transparent">
              {[
                { date: new Date().toLocaleDateString(), type: 'SALE', qty: -5, balance: product.stock, ref: 'INV-9901' },
                { date: new Date(Date.now() - 86400000).toLocaleDateString(), type: 'RESTOCK', qty: 20, balance: product.stock + 5, ref: 'PO-1001' },
                { date: new Date(Date.now() - 172800000).toLocaleDateString(), type: 'SALE', qty: -2, balance: product.stock - 15, ref: 'INV-9850' },
              ].map((move, i) => (
                <div key={i} className="list-group-item bg-transparent border-secondary border-opacity-10 px-0 py-3 d-flex justify-content-between align-items-center">
                  <div className="d-flex align-items-center gap-3">
                    <div className={`rounded-circle p-2 ${move.type === 'SALE' ? "bg-danger bg-opacity-10 text-danger" : "bg-success bg-opacity-10 text-success"}`}>
                      {move.type === 'SALE' ? <BarChart3 size={18} /> : <Package size={18} />}
                    </div>
                    <div>
                      <p className="text-dark fw-bold mb-0 small">{move.type}</p>
                      <p className="small text-muted mb-0">{move.date} {'\u2022'} Ref: {move.ref}</p>
                    </div>
                  </div>
                  <div className="text-end">
                    <p className={`fw-bold mb-0 small ${move.qty > 0 ? "text-success" : "text-danger"}`}>
                      {move.qty > 0 ? `+${move.qty}` : move.qty}
                    </p>
                    <p className="small text-muted mb-0">Bal: {move.balance}</p>
                  </div>
                </div>
              ))}
            </div>
