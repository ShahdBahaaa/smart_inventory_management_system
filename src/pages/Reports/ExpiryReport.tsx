...
const ExpiryReport = () => {
    ...
    const handleExport = (format: 'csv' | 'pdf') => {
        if (format === 'pdf') {
            const doc = new jsPDF();
            doc.text("Expiry Stability Report", 14, 15);
            autoTable(doc, {
                head: [['Batch/Lot Number', 'Product Name', 'Units Affected', 'Expiry Date', 'Status']],
                body: filteredData.map(b => {
                    ...
                    const productName = productsMap[b.productId]?.name || 'Unknown Product';
                    return [b.lotNumber, productName, b.quantity.toString(), new Date(b.expiryDate).toLocaleDateString(), status];
                }),
                startY: 20,
            });
