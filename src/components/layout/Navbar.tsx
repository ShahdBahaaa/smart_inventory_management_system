...
const Navbar = () => {
  ...
  const handleResultClick = (medicine: any) => {
    navigate(`/products/${medicine.id}`);
  };

  return (
    <header className="navbar navbar-expand-md sticky-top flex-md-nowrap p-0 border-bottom border-secondary border-opacity-10" style={{ background: 'rgba(255, 255, 255, 0.9)', backdropFilter: 'blur(10px)', zIndex: 1030 }}>
      <Link className="navbar-brand col-md-3 col-lg-2 me-0 px-4 fs-4 fw-bold d-flex align-items-center gap-2" to="/" style={{ color: '#00f2ff' }}>
        <Cpu size={24} />
        <span className="neon-text">SmartAI</span>
      </Link>
      <button className="navbar-toggler position-absolute d-md-none collapsed border-0" type="button" data-bs-toggle="collapse" data-bs-target="#sidebarMenu" aria-controls="sidebarMenu" aria-expanded="false" aria-label="Toggle navigation">
        ...
        <div className="navbar-nav flex-row align-items-center gap-3">
          <div className="nav-item dropdown">
            ...
            <ul className="dropdown-menu dropdown-menu-end glass-card border-secondary border-opacity-10 shadow-lg mt-2 p-0 overflow-hidden" style={{ width: '300px' }}>
              ...
              {lowStockMeds.length === 0 ? (
                ...
                lowStockMeds.map(med => (
                  <li key={med.id} className="border-bottom border-secondary border-opacity-5">
                    <Link to={`/products/${med.id}`} className="dropdown-item py-3 d-flex align-items-start gap-3 text-wrap">
              ...
              <li className="bg-light">
                <Link to="/products" className="dropdown-item py-2 text-center text-info small fw-bold">View All Inventory</Link>
              </li>
            </ul>
          </div>
          ...
          <div className="nav-item dropdown">
            ...
            <ul className="dropdown-menu dropdown-menu-end glass-card border-secondary border-opacity-10 shadow-lg mt-2" aria-labelledby="navbarDropdown">
              <li><Link className="dropdown-item py-2 text-dark opacity-75 hover-opacity-100" to="/profile">Operator Profile</Link></li>
              <li><Link className="dropdown-item py-2 text-dark opacity-75 hover-opacity-100" to="/settings">System Settings</Link></li>
              <li><hr className="dropdown-divider border-secondary border-opacity-10" /></li>
              <li><Link className="dropdown-item py-2 text-danger fw-bold" to="/login">Terminate Session</Link></li>
            </ul>
          </div>
        </div>
      </div>
